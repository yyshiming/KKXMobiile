...
open class KKXScanView: UIView {
    
    // 扫码区域各种参数
    var viewStyle = KKXScanView.Style()

    // 扫码区域
    var scanRetangleRect = CGRect.zero

    // 线条扫码动画封装
    var scanLineAnimation: KKXScanLineAnimation?

    // 网格扫码动画封装
    var scanNetAnimation: KKXScanNetAnimation?
    ...
    */
    public init(frame: CGRect, vstyle: KKXScanView.Style) {
        viewStyle = vstyle

        switch viewStyle.anmiationStyle {
        case .lineMove:
            scanLineAnimation = KKXScanLineAnimation()
        case .netGrid:
            scanNetAnimation = KKXScanNetAnimation()
        case .lineStill:
    ...
    func drawScanRect() {
        ...
        var sizeRetangle = CGSize(width: frame.width - XRetangleLeft * 2.0, height: frame.width - XRetangleLeft * 2.0)
        ...
        // 扫码区域Y轴最小坐标
        let YMinRetangle = frame.height / 2.0 - sizeRetangle.height / 2.0 - viewStyle.centerUpOffset
        let YMaxRetangle = YMinRetangle + sizeRetangle.height
        let XRetangleRight = frame.width - XRetangleLeft
        ...
        // 扫码区域下面填充
        rect = CGRect(x: 0, y: YMaxRetangle, width: frame.width, height: frame.height - YMaxRetangle)
        context.fill(rect)
        // 执行绘画
        context.strokePath()
    ...
    // 根据矩形区域，获取识别区域
    static func getScanRectWithPreView(preView: UIView, style: KKXScanView.Style) -> CGRect {
        let XRetangleLeft = style.xScanRetangleOffset
        let width = preView.frame.width - XRetangleLeft * 2
        let height = width
        ...
        // 扫码区域Y轴最小坐标
        let YMinRetangle = preView.frame.height / 2.0 - sizeRetangle.height / 2.0 - style.centerUpOffset
        // 扫码区域坐标
    ...
    func getRetangeSize() -> CGSize {
        let XRetangleLeft = viewStyle.xScanRetangleOffset
        var sizeRetangle = CGSize(width: frame.width - XRetangleLeft * 2, height: frame.width - XRetangleLeft * 2)
    ...
    func deviceStartReadying(readyStr: String) {
        ...
        if activityView == nil {
            ...
            let labelReadyRect = CGRect(x: activityView!.frame.origin.x + activityView!.frame.width + 10,
...
//MARK: - 公开方法
public extension KKXScanView {
    
    /// 获取扫描动画的Rect
    func getScanRectForAnimation() -> CGRect {
        let XRetangleLeft = viewStyle.xScanRetangleOffset
        var sizeRetangle = CGSize(width: frame.width - XRetangleLeft * 2,
                                  height: frame.width - XRetangleLeft * 2)

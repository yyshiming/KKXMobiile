...
open class KKXAlertController: KKXViewController {
    ...
    /// 点击按钮后是否自动dismiss, 默认true，style为cancel的按钮总是自动取消
    ...
    public func addAction(_ action: KKXAlertAction) {
        let button = KKXActionButton(type: .custom)
    ...
    private var actionButtons: [KKXActionButton] = []
    ...
    open override func traitCollectionDidChange(_ previousTraitCollection: UITraitCollection?) {
        ...
        actionButtons.forEach { $0.setBackgroundImage(UIColor.highlightBackground.image, for: .highlighted) }
    }
    ...
    @objc private func actionButtonClicked(_ sender: KKXActionButton) {
        sender.action?.actionHandler()
        if autoDismiss || sender.action?.style == .cancel {
            cancelAction()
        }
    }
...
public class KKXAlertAction: NSObject {
    
    public var title: String?
    private(set) var style: KKXAlertAction.Style = .default
    public var attributedTitle: NSAttributedString?
    
    private var handler: ((KKXAlertAction) -> Void)?
    
    convenience override init() {
        self.init(title: nil, style: .default)
    }
    
    public init(title: String?, style: KKXAlertAction.Style, handler: ((KKXAlertAction) -> Void)? = nil) {
        self.title = title
        self.style = style
        self.handler = handler
    }
    
    public init(attributedTitle: NSAttributedString?, style: KKXAlertAction.Style, handler: ((KKXAlertAction) -> Void)? = nil) {
        self.attributedTitle = attributedTitle
        self.style = style
        self.handler = handler
    }
    
    fileprivate func actionHandler() {
        handler?(self)
    }
}

extension KKXAlertAction {
...
private class KKXActionButton: UIButton {
    
    var action: KKXAlertAction? {
        didSet {
            guard let action = action else {
                return
            }
            var attributedTitle = action.attributedTitle
            if attributedTitle == nil, let string = action.title {
                let fontSize: CGFloat = 17
                var font: UIFont = .systemFont(ofSize: fontSize)
                var foregroundColor: UIColor = .kkxSystemBlue
                switch action.style {
                case .cancel:
